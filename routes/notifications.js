...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (category === "unread") {
      ...
      categoryData.question = categoryData.question?.filter(
        (notif) => !notif.isRead
      );
      categoryData.global = categoryData.global?.filter(
        (notif) => !notif.isRead
      );
    } else if (category === "read") {
      categoryData.todo = categoryData.todo?.filter((notif) => notif.isRead);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const categoryCounts = {
      ...
      legacy: await Notification.countDocuments({
        recipient: userId,
        isRead: false,
      }),
    };
    ...
    res.json({
      success: true,
      categoryCounts,
      total: Object.values(categoryCounts).reduce(
        (sum, count) => sum + count,
        0
      ),
    });
